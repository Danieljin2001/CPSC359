...
#Daniel Jin 30107081
#This code allows the user to use a keyboard or joystick for input
#Assignment 3


import pygame, os, random
from pygame.locals import *
import explorerhat as exh #imports explorer hat so joystick can be used
...
class Ship(pygame.sprite.Sprite):
    ...
    def update(self):
        x = exh.analog.one.read() #value indicating whether the position of the joystick is more left or more right
        button = exh.input.one.read() #value indicating whether the joystick button is pressed or not (pressed = 1, not pressed = 0)
        key = pygame.key.get_pressed()
        if key[K_LEFT] or x >= 4.5: #if left key is pressed or joystick is pulled left (where left is around 5, centre is around 2.5, right is 0)
            self.rect.move_ip(-5, 0)
        if key[K_RIGHT] or x <= 0.5: #if right key is pressed or joystick is pulled right
            self.rect.move_ip(5, 0)

        self.reload_timer += 1
        if (key[K_SPACE] and not self.overheated) or (button == 1 and not self.overheated): #if space bar and ship isn't overheated or button is pressed and ship is not overheated
            self.heat += 0.75
...
class Game:
    ...
    def menuLoop(self):
        ...
        while 1:
            ...
            self.bg.update()
            self.clock.tick(60)
            y = exh.analog.two.read() #value indicating whether the position of the joystick is more up or more down
            button = exh.input.one.read() #value indicating whether the joystick button is pressed or not (pressed = 1, not pressed = 0)
            if y <= 0.5: #if joystick is pulled down (where down is around 0, centre is around 2.5, up is 5)
                option = 2
            if y >= 4.5: #if joystick is pulled up
                option = 1
