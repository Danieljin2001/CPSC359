...
#Daniel Jin
#30107081
#Assignment 3. This program lets user use the mpu6050 for input


import pygame, os, random
from pygame.locals import *
from mpu6050 import mpu6050 #import mpu6050 so that this program can use it

random.seed()
WORLD = Rect(0, 0, 480, 550)
sensor = mpu6050(0x68) #sensor of mpu6050
...
class Ship(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        accel_data = sensor.get_accel_data() #accel data (x,y,z)
        if key[K_LEFT] or accel_data['x'] >= 2.5: #if left key is pressed or x value is greater or equal to 2.5 (mpu is turned to the left)
            self.rect.move_ip(-5, 0)
        if key[K_RIGHT] or accel_data['x'] <= -2.5: #if right key is pressed or x value is lesser or equal to -2.5 (mpu is turned to the right)
            self.rect.move_ip(5, 0)

        self.reload_timer += 1
        if (key[K_SPACE] and not self.overheated) or (accel_data['y'] <= -2.5 and not self.overheated): #if space bar and ship is not overheated or y value is less than or equal to -2.5 (tilted foward) and ship is not over heated
            self.heat += 0.75
...
class Game:
    ...
    def menuLoop(self):
        ...
        while 1:
            ...
            if accel_data['y'] >= 5:#if y value is greater or equal to 5 (mpu is turned towards user)
                option = 2
            if accel_data['y'] <= -5:#if y value is lesser or equal to -5 (mpu is turned away from user)
                option = 1
            if accel_data['x'] <= -8:#if x value is lesser or equal to -8 (mpu is turned right)
                if option == 1:
                    self.gameLoop()
                if option == 2:
                    pygame.quit()
