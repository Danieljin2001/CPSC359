...
#Assignment 4
#This version of the game can use the US100 to move the ship
#Daniel Jin 30107081
...
import US100    #import US100 module

random.seed()
WORLD = Rect(0, 0, 480, 550)
sensor = US100.Ultra()      #sensor initializes the Ultra class in US100
timer = US100.Clock()       #timer initializes the Clock class in US100

#sleep method (in mircoseconds)
def mirco_Sleep(timer, wait):
    t1 = timer.getTimer()
    t2 = timer.getTimer()
    while t2 - t1 < wait:
        t2 = timer.getTimer()

#method that returns the distance of object from the US100
def get_US100_data(sensor, timer):
    sensor.TX_on() #set TX pin to high
    mirco_Sleep(timer, 100) #set TX to high for 100 micro seconds
    sensor.TX_off() #set TX pin to low
    
    #wait for RX pin to be high
    while (sensor.read_RX() == 0):
        mirco_Sleep(timer, 1) #wait for 1 mirco second
        
    t1 = timer.getTimer() #record initial time
    
    #wait for RX pin to be low
    while (sensor.read_RX() == 1):
        mirco_Sleep(timer, 1) #wait for 1 micro second
        
    t2 = timer.getTimer() #record final time
    timeTaken = (t2 - t1) * 0.000001 #time it took (in seconds)
    distance = (timeTaken/2) * 34000 #distance travelled in centimeters
    return distance
...
class Ship(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        US100_data = get_US100_data(sensor, timer) #get data from US100
        if key[K_LEFT] or (US100_data >= 35 and US100_data <= 45): #if left key is pressed or distance of object from US100 is in between 25 and 55, move ship left
            self.rect.move_ip(-5, 0)
        if key[K_RIGHT] or (US100_data < 15 and US100_data >= 5): #if right key is pressed or distance of object from US100 is in between 3 and 24, move ship right
            self.rect.move_ip(5, 0)
